...
local options = { return_callback_errors = false }
...
local function await(fn, query, parameters)
    ...
    fn(nil, query, parameters, function(result, error)
        if error then return p:reject(error) end
        p:resolve(result)
...
local function safeArgs(query, parameters, cb, transaction)
    ...
    elseif transaction then
        if queryType ~= 'table' then error(("First argument expected table, received '%s'"):format(query)) end
